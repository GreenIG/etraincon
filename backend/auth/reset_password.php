...
// FOR DEBUGGING: If you still get a 500 error, uncomment these lines
...
if (empty($token)) {
    ...
    if ($user) {
        // Token is valid, so we can show the password reset form
        $show_form = true;
    } else {
...
// This part handles the form submission when the user enters a new password
if ($_SERVER["REQUEST_METHOD"] == "POST" && $show_form) {
    ...
    if (empty($password) || empty($confirm_password)) {
        ...
        // All checks passed, let's update the password
        ...
        if ($update_stmt->execute([$hashed_password, $user['id']])) {
            ...
            $show_form = false; // Hide the form after the password is changed
        } else {
